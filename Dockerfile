...
# and `pip install carlosversity` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "carlosversity", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n carlosversity -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "carlosversity", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "carlosversity", "YOUR_CONSOLE_SCRIPT"]
